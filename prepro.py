...
import numpy as np
import random
docred_rel2id = json.load(open('dataset/docred/label_map.json', 'r'))
...
def correct(sample, type):
    #random remove the sentence without any entity.
    if type == 'train':
        p = p_train
    else:
        p = p_dev
    ...
    ents = sample['vertexSet']
    sents = sample['sents']
    title = sample['title']
    newsents = []

    sents_id = [1] * len(sents)
    old2new = [0] *len(sents)
    for ent in ents:
        for m in ent:
            ent_sent.add(m['sent_id'])
    sent_id = list(ent_sent)
    for sen in sent_id:
        sents_id[sen] = 0
    for sen in sent_id:
        if sents_id[sen] == 1 and random.randint(0,9) < p:
            sents_id[sen] = 0
    idx = -1
    for i, rm in enumerate(sents_id):
        if rm :
            old2new[i] = -1
        else:
            newsents.append(sents[i])
            idx += 1
            old2new[i] = idx
    new_ents = []
    ...
    for ent in ents:
        newent = []
        for m in ent:
            m["sent_id"] = old2new[m["sent_id"]]
            if m["sent_id"] == -1:
                continue
            else:
                newent.append(m)
        new_ents.append(newent)
    temp = {'vertexSet':new_ents,  'sents':newsents, 'title':title}
    if 'labels' in sample.keys():
        temp['labels']=labels
    
    return temp

def read_docred(file_in, tokenizer, max_seq_length=1024, type='train'):
    i_line = 0
    pos_samples = 0
    neg_samples = 0
    features = []
    ...
    for sample in tqdm(data, desc="Example"):
        ...
        ht_i = np.array(hts)
        pairs = len(ht_i)
        ht_visible_mask = np.zeros((pairs, pairs), dtype='bool')
        for i in range(pairs):
            ht_visible_mask[i] = (ht_i[:, 0] == ht_i[i, 0]) + (ht_i[:, 1] == ht_i[i, 0]) + (ht_i[:, 0] == ht_i[i, 1]) + (ht_i[:, 1] == ht_i[i, 1])
        ...
        feature = {'input_ids': input_ids,
                   ...
                   'labels': relations,
                   'hts': hts,
                   'ht_visible_mask': ht_visible_mask,
                   'title': sample['title'],
                   }
...
def read_cdr(file_in, tokenizer, max_seq_length=1024, type='train'):
    pmids = set()
    features = []
    maxlen = 0
            ...
            if pmid not in pmids:
                ...
                for p in prs:
                    ...
                    if (h_id, t_id) not in train_triples:
                        train_triples[(h_id, t_id)] = [{'relation': r, 'cross': cross}]
                    else:
                        train_triples[(h_id, t_id)].append({'relation': r, 'cross': cross})

                relations, hts, intersent = [], [], []
...
def read_gda(file_in, tokenizer, max_seq_length=1024, type='train'):
    pmids = set()
    features = []
    maxlen = 0
    with open(file_in, 'r') as infile:
